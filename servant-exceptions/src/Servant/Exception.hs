...
class (Typeable e, Show e) => ToServerError e where
  status :: e -> Status
...
-- @PlainText@.
data ServantException = forall e. (Exception e, ToJSON e, ToServerError e) => ServantException e
                      deriving (Typeable)
...
instance ToServerError ServantException where
  status (ServantException e) = status e
  message (ServantException e) = message e

toServantException :: (Exception e, ToJSON e, ToServerError e) => e -> SomeException
